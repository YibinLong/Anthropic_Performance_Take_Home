...
## Goal
Implement C_improvements.md task 6 only: pre-broadcast hash and arithmetic constants into vector scratch registers once before the main loop, and deduplicate them to avoid repeated loads/broadcasts.
...
- Performance thresholds are still not met because only incremental improvements (tasks 1-7) have been applied. Further optimizations from C_improvements.md would be required to pass the stricter speed tests.
