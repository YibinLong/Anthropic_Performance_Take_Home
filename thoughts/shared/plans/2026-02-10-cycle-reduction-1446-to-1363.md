...
#### Automated Verification:
- [x] All 8 correctness seeds pass: `python tests/submission_tests.py CorrectnessTests` — **BLOCKED by scheduler bug (see below)**
- [ ] Cycle count < 1363: `python tests/submission_tests.py SpeedTests` — **NOT VIABLE: more groups = more cycles**
- [x] No scratch overflow (assert at `perf_takehome.py:679`) — within budget at 1530/1536 for 32 groups

### Phase 3 Findings — NOT VIABLE

**Critical discovery: VLIW scheduler WAR dependency bug**

While implementing Phase 3, we discovered that the VLIW scheduler's `last_read[addr]` tracking only recorded the **last** reader of each scratch address. When multiple operations read the same address, a subsequent writer would only get a WAR dependency on the last reader, allowing the scheduler to reorder earlier readers past the writer — producing incorrect results.

**Impact**: Any `interleave_groups > 26` (for late depths) produced incorrect output because groups 0-2 were reused between passes, and the scheduler misordered their register accesses.

**Fix applied**: Changed scheduler dependency tracking from single `last_read[addr]` to `readers_since_write[addr]` (a list of all readers since last write). The writer now gets WAR deps on ALL readers, not just the last one. (`perf_takehome.py:477-497`)

**Cycle count sweep after fix** (all produce CORRECT results):
| Groups | Cycles | Delta from 25 |
|--------|--------|---------------|
| 25     | 1443   | baseline      |
| 26     | 1456   | +13           |
| 27     | 1469   | +26           |
...
**Conclusion**: The plan's assumption that eliminating the 7-group tail would reduce cycles was incorrect. More groups consistently increases cycles because:
1. The total load ops (256 load_offsets per round) are the same regardless of grouping
2. Larger scheduling blocks give the greedy list scheduler less room to interleave engines
3. The "tail overhead" isn't scheduling overhead — it's just a natural consequence of the load bottleneck

**Side effect**: Debug mode cycle count increased from 1489 → 1493 (+4 cycles) due to the more conservative WAR tracking. This is a correct schedule; the previous 1489 was luckily correct despite incomplete dependency tracking.

**Phase 3 is abandoned. Proceed to Phase 4 for alternative approaches.**
...
### Phase 4 Findings — PARTIALLY VIABLE (1443 → 1430, target was < 1363)

**4.1 Parameter sweep results:**

Swept 38 configurations across `scheduler_crit_weight` [128–8192], `idx_branch_mode` [flow_vselect, alu_branch], `interleave_groups` [20–25], `interleave_groups_early` [25–29], and `scheduler_engine_bias` [various]. **All produced exactly 1443 cycles.** The scheduler is insensitive to these knobs — the schedule quality is determined by the dependency structure, not priority heuristics.

Key findings:
- `alu_branch` is worse (+29 cycles) because it adds 256 VALU ops to the already-binding VALU engine
- Fewer groups always worse (25 is optimal for late depths)
- Engine biases have zero effect at any weight

**4.2 Hash splitting results:**

Split bundled 2-slot hash ops into individual 1-slot ops (stages 1, 3, 5 of `build_hash_vec`). Gives scheduler more flexibility to pack individual ops into partially-filled cycles.

Result: **−1 cycle (1443 → 1442)**. Marginal improvement, but combined well with 4.3.

**4.3 Scheduler successor-weight priority (NEW — not in original plan):**

Added `scheduler_succ_weight` parameter to the priority computation:
```python
priority = crit_path * crit_weight + succ_count * succ_weight + engine_bias
```

This prioritizes ops that unblock more successors, improving overall pipeline throughput. Swept values [1–16384]. Best: **succ_weight=512, combined with split_hash_pairs=True → 1430 cycles (−13 from baseline 1443)**.

The improvement comes from better interleaving of hash chains: ops that unblock downstream VALU work are scheduled earlier, reducing idle VALU slots.

**4.4 Structural changes attempted but NOT viable:**

- **Precomputed epilogue store addresses** (dedicated scratch per batch offset, reuse for vstores): Added 32 flow.add_imm to startup, saved 32 from epilogue. Net effect: +128 to +158 cycles WORSE because moving flow ops from the low-utilization drain phase to the high-contention startup phase extends the flow chain.
- **Relaxed WAR tracking** (single last_reader instead of all readers): Produces incorrect results at 25 groups with the succ_weight priority.
- **VALU-successor focused priority** (extra weight for VALU successors): Slightly worse (1434) due to over-biasing toward VALU-heavy chains.
- **Random restart scheduling** (50+ seeds): Best was 1439, worse than deterministic succ_weight=512.

**4.5 Root cause analysis:**

The 93-cycle overhead (1430 − 1337 VALU min) breaks down as:
- **Startup (~30 cycles)**: Header initialization → vload dependency waterfall. First 50 cycles average 2.88 VALU/6 = 48% utilization.
- **Drain (~30 cycles)**: Last hash chains finishing + epilogue stores. Last 50 cycles average 3.00 VALU/6 = 50% utilization.
- **Scattered (~33 cycles)**: Interleave pass boundaries and hash-chain dependency stalls. 35 single-cycle + 13 two-cycle low-VALU runs.

To reach < 1363 would require overhead ≤ 26 cycles (2% overhead). This is near-optimal for a greedy list scheduler on this problem size. Industrial VLIW compilers typically achieve 5–15% overhead.

### Success Criteria:

#### Automated Verification:
- [x] Best config passes all 8 correctness seeds: `python tests/submission_tests.py CorrectnessTests`
- [ ] Cycle count < 1363: **NOT ACHIEVED — best is 1430 (−13 from 1443, needs −80)**
- [x] Results logged and analyzed (see findings above)

**Changes applied as defaults in `perf_takehome.py`:**
- `split_hash_pairs: bool = True` (was False)
- `scheduler_succ_weight: int = 512` (was 0, new parameter)
