...
## Ranked Improvements (Most Impactful -> Least)

1) **Real VLIW scheduler / slot packer (A1, B2, B13)**
...
2) **SIMD vectorization of the inner loop + gather via `load_offset` (A3, A6, B1)**
...
3) **Software pipelining / multi-group interleaving (A7, B7, B11)**
...
4) **Hash stage pair/combine parallelism (B4)**
...
5) **Eliminate flow selects with arithmetic (parity + wrap) (A4, A5, B6)**
...
6) **Pre-broadcast and reuse constants (A8, B5)**
...
8) **Loop structure decision (looped vs unrolled) (A12, B3, B12, B16)**
...
9) **Remove pause/debug in submission path (A10, B14)**
...
14) **Small arithmetic simplifications (A15, B15)**
